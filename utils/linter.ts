...
export function lintAndFixAST(ast: any, code: string): { errors: LintError[]; fixedCode: string } {
  ...
  // Get the current linter settings
  const { rules } = useLinterSettings.getState();
  ...
  function traverse(node: any, parent: any = null) {
    ...
    // Rule 1: Missing Semicolon - Auto-Fix (only if enabled)
    if (rules.missingSemicolon && node.type === "ExpressionStatement" && parent?.type !== "ForStatement") {
    ...
    // Rule 2: Undeclared Variables - No Auto-Fix (only if enabled)
    if (
      rules.undeclaredVariables &&
  ...
  // Report unused variables as errors (only if enabled)
  if (rules.unusedVariables) {
  ...
  // Remove unused variable declarations (only if enabled)
  function removeUnusedDeclarations(node: any, parent: any) {
    if (rules.unusedVariables && node.type === "VariableDeclaration") {
  ...
  function applyFixes(node: any) {
    if (rules.missingSemicolon && node.extra?.needsSemicolon) {
